...
def get_data(scene_file, mask_file, prefix, target, tokenizer, image_transform):
    max_text_len = 40
    ...
    data = image_transform(data) #SUSPECTED
...
def get_multi_scale_image_transform(cfg, is_train, get_one=get_transform_image):
    def get_multi_res_transform(s):
        ...
        t = get_one(cfg, is_train)
        t = ImageTransform2Dict(t, key='scene')
        all_t.append(t)
        t = get_one(cfg, is_train)
        t = ImageTransform2Dict(t, key='mask')
        all_t.append(t)
        if is_train:
...
def forward_backward_example(scene_files, mask_files, captions, prefixs=None):
    if prefixs is None:
...
def speed_test_forward_backward():
    duplicate = 32
    scene_files = ['aux_data/images/1.jpg', 'aux_data/images/2.jpg'] * duplicate
    mask_files = ['aux_data/images/1.jpg', 'aux_data/images/2.jpg'] * duplicate # Keeping these same as scene files for now.. replace with actual mask files
    ...
    logging.info(image_transform)
    data = recursive_to_device(data, 'cuda')
    data['scene'] = data['scene'].to(torch.float16)
    data['mask'] = data['mask'].to(torch.float16)
    ...
    for iteration in range(1000):
        ...
        if (iteration % 10) == 0:
            ...
            speed = data['scene'].shape[0] * 100 / (end - start)
            if iteration > 0:
