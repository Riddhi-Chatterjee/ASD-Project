...
def create_scene_mask_fusion_layer(visual_feature_size, 
                              fusion_type=None
                              ):
    if fusion_type is None:
        scene_mask_fusion = nn.Linear(
            2*visual_feature_size, visual_feature_size
        )
    elif fusion_type == "linearLn":
        scene_mask_fusion = nn.Sequential(
            nn.Linear(
                2*visual_feature_size, visual_feature_size
            ),
        ...
        raise NotImplementedError(fusion_type)
    return scene_mask_fusion
...
class TransformerDecoderTextualHead(TextualHead):
    ...
    def __init__(
        ...
        if visual_feature_size is not None:
            ...
            self.scene_mask_fusion = create_scene_mask_fusion_layer(visual_feature_size, fusion_type)
        else:
            self.visual_projection = nn.Identity()
            self.scene_mask_fusion = nn.Identity()
    ...
    def forward(
        ...
        projected_visual_features = self.visual_projection(self.scene_mask_fusion(hidden_states)) if hidden_states is not None else None
        if return_dict:
...
class CaptioningModel(nn.Module):
    ...
    def forward_one(self, batch, return_info=False):
        # shape: (batch_size, max_caption_length, vocab_size)
        if 'scene' in batch and 'mask' in batch:
            if isinstance(batch['scene'], (list, tuple)) and isinstance(batch['mask'], (list, tuple)):
                scene_features = [self.scene_encoder(im) for im in batch['scene']]
                mask_features = [self.mask_encoder(im) for im in batch['mask']]
                features = [torch.cat((scene, mask), dim=-1) for scene, mask in zip(scene_features, mask_features)] # Concatenating scene and mask features 
                ...
                scene_features = self.scene_encoder(batch['scene'])
                mask_features = self.mask_encoder(batch['mask'])
                visual_features = torch.cat((scene_features, mask_features), dim=-1)
        else:
    ...
    def forward_one_ce(self, batch, visual_features, visual_features_valid, return_info):
        ...
        assert has_image == (('scene' in batch) and ('mask' in batch))
        if self.training:
